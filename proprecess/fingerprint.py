...
if __name__ == '__main__':
    ...
    def generate_fingerprints(smiles):
        ...
        morgan_fp = list(GetMorganFingerprintAsBitVect(mol, 6, 1024,useChirality=True))
        return maccs_fp, pubchem_fp, morgan_fp
    def process_csv(input_path, output_path):
        df = pd.read_csv(input_path)
        combined_fingerprints = []

        for smiles in df['smiles']:
            maccs, pubchem, morgan_fp= generate_fingerprints(smiles)
            if maccs is not None:
                combined = maccs +  morgan_fp +pubchem
                combined_fingerprints.append(combined)
            else:
                combined_fingerprints.append(None)

        df['fingerprint'] = combined_fingerprints
