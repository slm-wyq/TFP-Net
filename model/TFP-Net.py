...
class TFPNet(nn.Module):
    """
    Graph TransformerConv model for graph-level classification.
    """
...
# model = TFPNet(lib_size=lib_size, num_classes=len(class_list), fp_dim=len(train_fp[0]))
#
...
glycan_graphs = dataset_to_graphs(taxonomic_glycans.target.values.tolist(), abc,
                                  fingerprint_list=taxonomic_glycans.fp_list.tolist(), libr=lib)
glycan_loader = DataLoader(glycan_graphs, batch_size=32, shuffle=False)
model_ft = TFPNet(lib_size=lib_size, num_classes=len(class_list), fp_dim=2072)
model_ft.load_state_dict(torch.load(f'../weights/TFP-Net-kingdom.pt'))
...
for data in glycan_loader:
    ...
    pred, out = model_ft(x, edge_index_ori, edge_index_full, batch, fp, inference=True)
    res.append(out)
...
ax = sns.scatterplot(x=tsne_emb[:, 0], y=tsne_emb[:, 1], s=40, alpha=0.4,
                hue=taxonomic_glycans.kingdom.values.tolist(), palette='colorblind', rasterized=True,legend=True)
...
labels = taxonomic_glycans.kingdom.values.tolist()
df = pd.DataFrame({
    "x": tsne_emb[:, 0],
    "y": tsne_emb[:, 1],
    "kingdom": labels
